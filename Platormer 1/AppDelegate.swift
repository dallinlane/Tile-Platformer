...
func switchViewControllers (storyboardID : String) {
    if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene {
        if let window = windowScene.windows.first(where: { $0.isKeyWindow }) {
            ...
            // Instantiate the view controller using the identifier
            let destinationController = storyboard.instantiateViewController(identifier: storyboardID)
            
            // Set the root view controller to the instantiated view controller
